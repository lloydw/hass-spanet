...
class SpaBlowerFan(SpaEntity, FanEntity):
    ...
    @property
    def is_on(self):
        return self._blower().get("status", "off") != "off"

    @property
    def preset_mode(self):
        return PRESET_RAMP if self._blower().get("status") == "ramp" else None

    @property
    def percentage(self):
        blower = self._blower()
        if blower.get("status") != "vari":
            return 0
        return ranged_value_to_percentage(SPEED_RANGE, blower.get("speed") or 1)

    async def async_set_percentage(self, percentage: int):
        if percentage == 0:
            await self.coordinator.set_blower_mode(MODE_OFF)
            return
        speed = max(1, round(percentage_to_ranged_value(SPEED_RANGE, percentage)))
        await self.coordinator.set_blower_mode(MODE_VARIABLE, speed)

    async def async_set_preset_mode(self, preset_mode: str):
        if preset_mode == PRESET_RAMP:
            await self.coordinator.set_blower_mode(MODE_RAMP)
    ...
    async def async_turn_on(self, percentage=None, preset_mode=None, **kwargs):
        if preset_mode == PRESET_RAMP:
            ...
            await self.coordinator.set_blower_mode(
                MODE_VARIABLE, self._blower().get("speed") or SPEED_RANGE[1]
            )
